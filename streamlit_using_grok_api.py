...
# ========== CLEAN CODE FROM LLM ==========
def clean_code_block(code: str, language: str) -> str:
...
# ========== CALL LLM ==========
def call_llm(system_prompt, user_code, language, coding_doc=None, user_prompt=None):
    full_user_prompt = ""
...
# ========== EXECUTE TESTS ==========
def run_tests(code_str: str, test_str: str, language: str) -> str:
    with tempfile.TemporaryDirectory() as temp_dir:
        if language.lower() == "python":
            ...
            with open(code_path, "w") as f:
                f.write(code_str)
            with open(test_path, "w") as f:
                # Insert a require to use the main.js code in test file
            ...
            return f"❌ Language '{language}' is not supported yet for test execution."
...
if col1.button("🔧 Standardize Code"):
    if not user_code or not system_prompt:
        ...
        raw_code = call_llm(system_prompt, user_code, language, coding_doc=coding_doc_content, user_prompt=user_prompt)
        clean_code = clean_code_block(raw_code, language)
...
if col2.button("🧪 Generate & Run Tests"):
    if "standardized_code" not in st.session_state:
        ...
        raw_tests = call_llm(system_prompt, st.session_state["standardized_code"], language, user_prompt=test_prompt)
        clean_tests = clean_code_block(raw_tests, language)

        st.subheader("🧪 Test Cases")
        st.code(clean_tests, language=language)

        st.subheader("📊 Test Execution Report")
        result = run_tests(st.session_state["standardized_code"], clean_tests, language)
        st.text(result)
